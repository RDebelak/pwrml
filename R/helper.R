...
coef_short = function(mirtfit,itemtype=NULL) {
  ...
  if (mirtfit@Data$ngroups==1) {
    ...
    if (itemtype=="gpcm") { # Other Form for GPCM
      re =  list(a = a$a1, d = as.matrix(a[grep("d",names(a))]),g = rep(0,length(a$a1)),itemtype=itemtype)
    } else if (ismulti) { # Other Form for mulidimensional
      re =  list(a = as.matrix(a[grep("a",names(a))]), d = as.matrix(a[grep("d",names(a))]),g = rep(0,length(a$a1)),itemtype=itemtype)
    } else { # default form
...
pars.long = function(pars,itemtype,from.mirt=FALSE) {
  ...
  if(from.mirt) {
    if (itemtype=="2PL"& pars@Call[[1]]=="mirt::multipleGroup" & pars@Model$nest== pars@Model$nestpars - pars@Data$nitems -5) {
      pars = lapply(mirt::coef(pars,simplify=TRUE),function(x) x$items[,1:2] %>% t() %>% as.numeric())
      re = c(pars$A,pars$B[seq(12,length(pars$B),2)])
      } else if (itemtype=="2PL"& pars@Call[[1]]=="mirt::multipleGroup") {
        pars = lapply(mirt::coef(pars,simplify=TRUE),function(x) x$items[,1:2] %>% t() %>% as.numeric())
